...
__declspec(dllexport) void BuildLayer1Input_WNN(int dataSet, int sampleCnt, int l0sampleLen, int l1sampleLen, int tsfcnt, double* tsf, int l0cores, int l0predlen, int l1predlen, void*** l0log, double** l1target, double** oSample, double** oTarget) {
	int i, n0, s;
	...
	for (s = 0; s < sampleCnt; s++) {
		...
		for (n0 = 0; n0 < l0cores; n0++) {
			...
			for (i = 0; i < l0predlen; i++) R[i] = ((NN_Logs*)l0log[n0][dataSet])->RunOutput[l0sampleLen + s + i].Predicted_TRS;	// we only need the last (prediction) from runlog
...
__declspec(dllexport) void BuildLayerInput_WNN(int layer, tEngineDef* pEngineParms, int dataTrx, int dataSet, int sampleCnt, int l0tslen, double* l0its, double l0ibv, double** l0ots, double* l0obv, double* tsf, void*** l0log, double** l1target, double** oSample, double** oTarget) {
	if(layer==0){
