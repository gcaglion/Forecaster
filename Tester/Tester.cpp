...
int main(int argc, char** argv) {
	...
	tForecastParms* fParms = new tForecastParms;
	//-- a. set overrides from Command Line 
	if (CLProcess(argc, argv, fParms->iniParms) <0) return -1;
	//-- b. process ini file
	if (ForecastParamLoader(fParms) <0) return -1;

	//-- Tester-specific parameters
	if (getParam(fParms->iniParms, "Tester.SimulationLength", &fParms->SimulationLength) < 0)		return -1;
	if (getParam(fParms->iniParms, "Tester.SimulationStart", &fParms->SimulationStart[0]) < 0)	return -1;
	if (getParam(fParms->iniParms, "Tester.PauseAtEnd", &FinalPause) <0)							return -1;
