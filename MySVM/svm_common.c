...
# include "svm_kernel.h"           /* this contains a user supplied kernel */

long   verbosity;              /* verbosity level (0-4) */
long   kernel_cache_statistic;

EXPORT double classify_example(MODEL *model, DOC *ex)
...
EXPORT double classify_example_linear(MODEL *model, DOC *ex)
...
EXPORT double kernel(KERNEL_PARM *kernel_parm, DOC *a, DOC *b)
...
EXPORT double single_kernel(KERNEL_PARM *kernel_parm, SVECTOR *a, SVECTOR *b)
...
EXPORT SVECTOR *create_svector(svmWORD *words,char *userdefined,double factor) {
  SVECTOR *vec;
  long    fnum,i;
...
EXPORT SVECTOR *copy_svector(SVECTOR *vec)
{
  SVECTOR *newvec=NULL;
  if(vec) {
...
EXPORT void free_svector(SVECTOR *vec)
{
  if(vec) {
    free(vec->words);
    if(vec->userdefined)
...
EXPORT double sprod_ss(SVECTOR *a, SVECTOR *b)
...
EXPORT SVECTOR* sub_ss(SVECTOR *a, SVECTOR *b)
...
EXPORT SVECTOR* add_ss(SVECTOR *a, SVECTOR *b)
...
EXPORT SVECTOR* add_list_ss(SVECTOR *a)
...
EXPORT void append_svector_list(SVECTOR *a, SVECTOR *b)
...
EXPORT SVECTOR* smult_s(SVECTOR *a, double factor)
...
EXPORT int featvec_eq(SVECTOR *a, SVECTOR *b)
...
EXPORT double model_length_s(MODEL *model, KERNEL_PARM *kernel_parm)
...
EXPORT void clear_vector_n(double *vec, long int n)
{
  register long i;
  for(i=0;i<=n;i++) vec[i]=0;
}

EXPORT void add_vector_ns(double *vec_n, SVECTOR *vec_s, double faktor)
{
  register svmWORD *ai;
  ai=vec_s->words;
  while (ai->wnum) {
...
EXPORT double sprod_ns(double *vec_n, SVECTOR *vec_s)
{
  register double sum=0;
  register svmWORD *ai;
  ai=vec_s->words;
...
EXPORT void add_weight_vector_to_linear_model(MODEL *model)
...
EXPORT void free_example(DOC *example, long deep)
{
  if(example) {
    if(deep) {
      if(example->fvec)
...
EXPORT int write_model(char *modelfile, MODEL *model)
{
  FILE *modelfl;
  long j,i,sv_num;
  SVECTOR *v;
  ...
  for(i=1;i<model->sv_num;i++) {
    for(v=model->supvec[i]->fvec;v;v=v->next) {
      ...
      for (j=0; (v->words[j]).wnum; j++) {
		fprintf(modelfl,"%ld:%.8g ", (long)(v->words[j]).wnum, (double)(v->words[j]).weight);
      }
      fprintf(modelfl,"#%s\n",v->userdefined);
...
EXPORT MODEL *read_model(char *modelfile)
{
  FILE *modelfl;
  ...
  if (nol_ll(modelfile, &max_sv, &max_words, &ll) < 0) {
	  model->retcode = -1; return model;
  } 
  max_words+=2;
  ll+=2;
  ...
  {
	  perror(modelfile); model->retcode = -1; return model;
  }
...
EXPORT MODEL *copy_model(MODEL *model)
{
  MODEL *newmodel;
  long  i;
...
EXPORT void free_model(MODEL *model, int deep)
{
  long i;
...
EXPORT int read_documents(char *docfile, DOC ***docs, double **label,
...
EXPORT int parse_document(char *line, svmWORD *words, double *label,
		   ...
		   char **comment)
{
  ...
  while(((numread=sscanf(line+pos,"%s",featurepair)) != EOF) && (numread > 0) && (wpos<max_words_doc)) {
		/* printf("%s\n",featurepair); */
		while(space_or_null((int)line[pos])) pos++;
		while((!space_or_null((int)line[pos])) && line[pos]) pos++;
...
EXPORT double *read_alphas(char *alphafile,long totdoc)
        ...
        write_alphas function */
{
  ...
  if(dnum != totdoc)
  { perror ("\nNot enough values in alpha file!"); alpha[0] = -1; }
  fclose(fl);
...
EXPORT int nol_ll(char *file, long int *nol, long int *wol, long int *ll)
...
EXPORT long minl(long int a, long int b)
{
  if(a<b)
    return(a);
  else
...
EXPORT long maxl(long int a, long int b)
{
  if(a>b)
    return(a);
  else
...
EXPORT int space_or_null(int c) {
  if (c==0)
    return 1;
...
EXPORT void set_verbosity(long v) { verbosity = v; }
