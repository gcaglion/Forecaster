...
int typeofParam(T p) {
	char tdesc[10];
	boolean ispointer = (strlen(tdesc > 2));
	strcpy(tdesc, typeid(p).raw_name());
	char* tcode = right(tdesc, 1);
	//printf("typeid.hash_code is %d\n", typeid(p).hash_code());
	printf("typeid.name      is %s\n", typeid(p).name());
	//printf("typeid.raw_name  is %s (%s) -> %s\n", typeid(p).raw_name(), tdesc, tcode);
	if (strlen(tdesc > 2)) {
		if (strcmp(tcode, "H") == 0) {
			return PARAM_TYPE_INTp;
		}
		else if (strcmp(tcode, "N") == 0) {
			return PARAM_TYPE_DOUBLEp;
		}
		else if (strcmp(tcode, "D") == 0) {
			return PARAM_TYPE_STRINGp;
		}
	}
	else {
		if (strcmp(tcode, "H") == 0) {
...
template <typename T>
__declspec(dllexport) int __stdcall getParam(char* fileName, char* paramName, T* oparamVal, bool isenum = false) {
	char evals[100];
	char pname[1000];
	int t;
	...
	if (ReadParamFromFile(fileName, pname, evals) != 0) return -1;
	(*oparamVal) = getEnumVal(pname, evals);
	return 0;
}
*/
