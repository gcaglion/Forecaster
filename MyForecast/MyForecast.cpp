...
//--
/*
void* Malloc_XXX_Log(tEngineDef* pEngineParms, void* logPointer, int LayerId, int CoreId, int DatasetId, int pSampleCount) {
	void* ret = nullptr;
	NN_Parms* NNParms;
	SVM_Parms* SVMParms;

	switch (pEngineParms->CoreEngineType[LayerId][CoreId]) {
	case ENGINE_NN:
		NNParms = (NN_Parms*)pEngineParms->CoreSpecs[LayerId][CoreId];
		NNLogs = (NN_Logs*)malloc(sizeof(NN_Logs));	// logPointer;
		NNLogs->MSEOutput	= (tLogMSE*)malloc(pEngineParms->MSECount[LayerId][CoreId] * sizeof(tLogMSE));
		NNLogs->RunOutput	= (tLogRUN*)malloc(pEngineParms->RunCount[LayerId][CoreId] * sizeof(tLogRUN));
		NNLogs->IntP		= (tLogInt*)malloc(NNParms->MaxEpochs*pSampleCount * sizeof(tLogInt));
		NNLogs->FinalW = (tNNWeight***)malloc(NNParms->LevelsCount * sizeof(tNNWeight**));
		for (int nl = 0; nl<(NNParms->LevelsCount - 1); nl++) {
			NNLogs->FinalW[nl] = (tNNWeight**)malloc(NNParms->NodesCount[nl] * sizeof(tNNWeight*));
			for (int fnn = 0; fnn<NNParms->NodesCount[nl + 1]; fnn++) {
				NNLogs->FinalW[nl][fnn] = (tNNWeight*)malloc(NNParms->NodesCount[nl] * sizeof(tNNWeight));
			}
		}
		ret = (void*)NNLogs;
		//-- TODO: IntP, IntW, IntF, Inta
		break;
	case ENGINE_WNN:
		//-- identical to ENGINE_NN
		NNParms = (NN_Parms*)pEngineParms->CoreSpecs[LayerId][CoreId];
	...
	case ENGINE_SOM:
		break;
	case ENGINE_SVM:
		SVMParms = (SVM_Parms*)pEngineParms->CoreSpecs[LayerId][CoreId];
		SVMLogs = (SVM_Logs*)malloc(sizeof(SVM_Logs));	// logPointer;
		SVMLogs->MSEOutput = (tLogMSE*)malloc(pEngineParms->MSECount[LayerId][CoreId] * sizeof(tLogMSE));
		SVMLogs->RunOutput = (tLogRUN*)malloc(pEngineParms->RunCount[LayerId][CoreId] * sizeof(tLogRUN));
		ret = (void*)SVMLogs;
		break;
	}
...
void Free_XXX_Log(tEngineDef* pEngineParms, int LayerId, int CoreId, int DatasetId, void* log) {
	...
	case ENGINE_NN:
		NNLogs = (NN_Logs*)log;
		NNParms = (NN_Parms*)pEngineParms->CoreSpecs[0][0];

		free(NNLogs->MSEOutput);
		free(NNLogs->RunOutput);
		free(NNLogs->IntP);
		for (int nl = 0; nl<(NNParms->LevelsCount - 1); nl++) {
			for (int fnn = 0; fnn<NNParms->NodesCount[nl + 1]; fnn++) {
				free(NNLogs->FinalW[nl][fnn]);
			}
			free(NNLogs->FinalW[nl]);
		}
		free(NNLogs->FinalW);
		free(log);
		break;
	...
	case ENGINE_SOM:
		break;
	case ENGINE_SVM:
		SVMLogs = (SVM_Logs*)log;
		SVMParms = (SVM_Parms*)pEngineParms->CoreSpecs[LayerId][CoreId];

		free(SVMLogs->MSEOutput);
		free(SVMLogs->RunOutput);
		free(log);
		break;
	}
...
*/
/*
void Free_EngineParms(tEngineDef* pEngineParms, int pDatasetsCount) {

	//-- 1. Free Core Logs
	NN_Logs**** NNLogs = nullptr;
	GA_Logs**** GALogs = nullptr;
	SOM_Logs**** SOMLogs = nullptr;
	SVM_Logs**** SVMLogs = nullptr;
	void* coreLog = nullptr;
	for (int l = 0; l<pEngineParms->LayersCount; l++) {
		for (int n = 0; n<pEngineParms->CoresCount[l]; n++) {
			for (int d = 0; d<pDatasetsCount; d++) {

				switch (pEngineParms->EngineType) {
				case ENGINE_NN:
					NNLogs = (NN_Logs****)pEngineParms->CoreLogs;
					coreLog = (void*)NNLogs[l][n][d];
					break;
				case ENGINE_WNN:
					NNLogs = (NN_Logs****)pEngineParms->CoreLogs;
					coreLog = (void*)NNLogs[l][n][d];
					break;
				case ENGINE_GA:
					GALogs = (GA_Logs****)pEngineParms->CoreLogs;
					coreLog = (void*)GALogs[l][n][d];
					break;
				case ENGINE_SOM:
					SOMLogs = (SOM_Logs****)pEngineParms->CoreLogs;
					coreLog = (void*)SOMLogs[l][n][d];
					break;
				case ENGINE_SVM:
					SVMLogs = (SVM_Logs****)pEngineParms->CoreLogs;
					coreLog = (void*)SVMLogs[l][n][d];
					break;
				}
				Free_XXX_Log(pEngineParms, l, n, d, coreLog);
			}
			free(pEngineParms->CoreLogs[l][n]);
		}
		free(pEngineParms->CoreLogs[l]);
	}
	free(pEngineParms->CoreLogs);

	//-- 2. Free Engine Counters
	for (int l = 0; l<pEngineParms->LayersCount; l++) {
		free(pEngineParms->CoreEngineType[l]);
		free(pEngineParms->MSECount[l]);
		free(pEngineParms->TimeStepsCount[l]);
		free(pEngineParms->SampleLen[l]);
		free(pEngineParms->TargetLen[l]);
		free(pEngineParms->RunCount[l]);
	}
	free(pEngineParms->CoreEngineType);
	free(pEngineParms->MSECount);
	free(pEngineParms->TimeStepsCount);
	free(pEngineParms->SampleLen);
	free(pEngineParms->TargetLen);
	free(pEngineParms->RunCount);

}
*/
void NNsetNodesCount(NN_Parms* NNParms) {
	//-- calc nodescount[], totalnodescount from InputCount,OutputCount,LevelRatios
	int nl, nn0, nn1;
	NNParms->NodesCount[0] = NNParms->InputCount;
	NNParms->NodesCount[NNParms->LevelsCount - 1] = NNParms->OutputCount;
	NNParms->NodesCountTotal = NNParms->InputCount + NNParms->OutputCount;
	for (nl = 0; nl<(NNParms->LevelsCount - 2); nl++) {
		NNParms->NodesCount[nl + 1] = (int)floor(NNParms->NodesCount[nl] * NNParms->LevelRatio[nl]);
		NNParms->NodesCountTotal += NNParms->NodesCount[nl + 1];
	}
	//-- calc totalweightscount
	NNParms->WeightsCountTotal = 0;
	for (nl = 0; nl<(NNParms->LevelsCount - 1); nl++) {
		for (nn1 = 0; nn1<NNParms->NodesCount[nl + 1]; nn1++) {
			for (nn0 = 0; nn0<NNParms->NodesCount[nl]; nn0++) {
				NNParms->WeightsCountTotal++;
			}
		}
	}
}
/*
void CastEngineParms(tEngineDef* pEngineParms, tDataShape* pDataParms) {
	NN_Parms* NNParms;
	GA_Parms* GAParms;
	SOM_Parms* SOMParms;
	SVM_Parms* SVMParms;
	...
	//-- 1. set Engine Layout
	//-- 1. Set CoresCount[], TotalCoresCount, CoreEngineType[][]
	switch (pEngineParms->EngineType) {
	case ENGINE_WNN:
		WNNParms = (WNN_Arch*)pEngineParms->EngineParms;
		pEngineParms->CoresCount[0] = WNNParms->DecompLevel + 1;
		pEngineParms->CoresCount[1] = 1;
		pEngineParms->TotalCoresCount = WNNParms->DecompLevel + 1 + 1;
		break;
	case ENGINE_XIE:
		pEngineParms->CoresCount[0] = 2;
		pEngineParms->CoresCount[1] = 1;
		pEngineParms->TotalCoresCount = 3;
		break;
	default:
		pEngineParms->CoresCount[0] = 1;
		pEngineParms->TotalCoresCount = 1;
		break;
	}
	//-- 2. set Engine-specific, derived parameters (InputCount, OutputCount, NodesCount[], NodesCountTotal, WeightsCountTotal
	int l, n, d;
	NN_Parms* prev_NNParms;
	//SVM_Parms* prev_SVMParms;
	for (l = 0; l<pEngineParms->LayersCount; l++) {
		for (n = 0; n<pEngineParms->CoresCount[l]; n++) {
			for (d = 0; d<pDataParms->DatasetsCount; d++) {
				switch (pEngineParms->EngineType) {
				case ENGINE_NN:
					NNParms = (NN_Parms*)pEngineParms->CoreSpecs[l][n];
					NNParms->InputCount = pDataParms->SampleLen;
					NNParms->OutputCount = pDataParms->PredictionLen;
					NNsetNodesCount(NNParms);
					break;
				case ENGINE_WNN:
					NNParms = (NN_Parms*)pEngineParms->CoreSpecs[l][n];
					//-- 
					if (l>0) {
						prev_NNParms = (NN_Parms*)pEngineParms->CoreSpecs[l - 1][n];
						NNParms->InputCount = pEngineParms->CoresCount[l - 1] * prev_NNParms->OutputCount  + pEngineParms->TSFcnt ;	// output from layer 0 + TSFcnt
					}
					else {
						NNParms->InputCount = pDataParms->SampleLen;
					}
				...
				case ENGINE_SOM:
					break;
				case ENGINE_SVM:
					SVMParms = (SVM_Parms*)pEngineParms->CoreSpecs[l][n];
					SVMParms->InputCount = pDataParms->SampleLen;
					break;
				case ENGINE_XIE:
					if (l > 0) {
						NNParms = (NN_Parms*)pEngineParms->CoreSpecs[l][n];
						prev_NNParms = (NN_Parms*)pEngineParms->CoreSpecs[0][1];
						NNParms->InputCount = 2 * prev_NNParms->OutputCount  + pEngineParms->TSFcnt ;	// output from layer 0 + TSFcnt
						NNParms->OutputCount = pDataParms->PredictionLen;
						NNsetNodesCount(NNParms);
					} else {
						//-- at L0, we have SVM as core[0] , NN as core[1]
						if (n > 0) {
							NNParms = (NN_Parms*)pEngineParms->CoreSpecs[l][n];
							NNParms->InputCount = pDataParms->SampleLen;
							NNParms->OutputCount = pDataParms->PredictionLen;
	...
	//-- 3. set Engine Counters (EngineDef-> MSECount, TimeStepsCount, SampleCount, RunCount)

	//--- 3.1. malloc()s
	pEngineParms->CoreEngineType = (int**)malloc(pEngineParms->LayersCount * sizeof(int*));
	pEngineParms->MSECount = (int**)malloc(pEngineParms->LayersCount * sizeof(int*));
	pEngineParms->TimeStepsCount = (int**)malloc(pEngineParms->LayersCount * sizeof(int*));
	pEngineParms->SampleLen = (int**)malloc(pEngineParms->LayersCount * sizeof(int*));
	pEngineParms->TargetLen = (int**)malloc(pEngineParms->LayersCount * sizeof(int*));
	pEngineParms->RunCount = (int**)malloc(pEngineParms->LayersCount * sizeof(int*));
	for (int l = 0; l<pEngineParms->LayersCount; l++) {
		pEngineParms->CoreEngineType[l] = (int*)malloc(pEngineParms->CoresCount[l] * sizeof(int));
		pEngineParms->MSECount[l] = (int*)malloc(pEngineParms->CoresCount[l] * sizeof(int));
		pEngineParms->TimeStepsCount[l] = (int*)malloc(pEngineParms->CoresCount[l] * sizeof(int));
		pEngineParms->SampleLen[l] = (int*)malloc(pEngineParms->CoresCount[l] * sizeof(int));
		pEngineParms->TargetLen[l] = (int*)malloc(pEngineParms->CoresCount[l] * sizeof(int));
		pEngineParms->RunCount[l] = (int*)malloc(pEngineParms->CoresCount[l] * sizeof(int));
	}

	//--- 3.2. get values from parameters (engine-specific and data)
	for (int l = 0; l<pEngineParms->LayersCount; l++) {
		for (int n = 0; n<pEngineParms->CoresCount[l]; n++) {
			switch (pEngineParms->EngineType) {
			case ENGINE_NN:
				NNParms = (NN_Parms*)pEngineParms->CoreSpecs[l][n];
				pEngineParms->CoreEngineType[l][n] = ENGINE_NN;
				pEngineParms->MSECount[l][n] = NNParms->MaxEpochs;
				pEngineParms->TimeStepsCount[l][n] = NNParms->MaxEpochs;
				pEngineParms->SampleLen[l][n] = NNParms->InputCount;
				pEngineParms->TargetLen[l][n] = pDataParms->PredictionLen;
				pEngineParms->RunCount[l][n] = (pDataParms->HistoryLen + pDataParms->PredictionLen);
				break;
			case ENGINE_WNN:
				NNParms = (NN_Parms*)pEngineParms->CoreSpecs[l][n];
				pEngineParms->CoreEngineType[l][n] = ENGINE_NN;
				...
				if (l > 0) {
					pEngineParms->SampleLen[l][n] = 0;
					for (int pn = 0; pn < pEngineParms->CoresCount[l - 1]; pn++) pEngineParms->SampleLen[l][n] += pEngineParms->TargetLen[l - 1][pn];
				}
				else {
					pEngineParms->SampleLen[l][n] = NNParms->InputCount;
				}
				...
				pEngineParms->TargetLen[l][n] = pDataParms->PredictionLen;
				//pEngineParms->RunCount[l][n] = (pDataParms->HistoryLen + pDataParms->PredictionLen);
				pEngineParms->RunCount[l][n] = (pDataParms->SampleCount + NNParms->InputCount + NNParms->OutputCount);
				break;
			case ENGINE_GA:
				GAParms = (GA_Parms*)pEngineParms->CoreSpecs[l][n];
				pEngineParms->CoreEngineType[l][n] = ENGINE_GA;
				break;
			case ENGINE_SOM:
				SOMParms = (SOM_Parms*)pEngineParms->CoreSpecs[l][n];
				pEngineParms->CoreEngineType[l][n] = ENGINE_SOM;
				break;
			case ENGINE_SVM:
				SVMParms = (SVM_Parms*)pEngineParms->CoreSpecs[l][n];
				pEngineParms->CoreEngineType[l][n] = ENGINE_SVM;
				pEngineParms->MSECount[l][n] = SVMParms->MaxEpochs;
				pEngineParms->TimeStepsCount[l][n] = SVMParms->MaxEpochs;
				pEngineParms->SampleLen[l][n] = SVMParms->InputCount;
				pEngineParms->TargetLen[l][n] = pDataParms->PredictionLen;
				pEngineParms->RunCount[l][n] = (pDataParms->HistoryLen + pDataParms->PredictionLen);
				break;
			case ENGINE_XIE:
				if (l > 0) {
					...
					pEngineParms->SampleLen[l][n] = pDataParms->SampleLen; 
					if (n > 0) {
						NNParms = (NN_Parms*)pEngineParms->CoreSpecs[l][n];
						pEngineParms->CoreEngineType[l][n] = ENGINE_NN;
						pEngineParms->MSECount[l][n] = NNParms->MaxEpochs;
	...
	//-- 4. set CoreLogs
	pEngineParms->CoreLogs = (void****)malloc(pEngineParms->LayersCount * sizeof(void***));
	for (int l = 0; l<pEngineParms->LayersCount; l++) {
		pEngineParms->CoreLogs[l] = (void***)malloc(pEngineParms->CoresCount[l] * sizeof(void**));
		for (int n = 0; n<pEngineParms->CoresCount[l]; n++) {
			pEngineParms->CoreLogs[l][n] = (void**)malloc(pDataParms->DatasetsCount * sizeof(void*));
			for (int d = 0; d<pDataParms->DatasetsCount; d++) {
				pEngineParms->CoreLogs[l][n][d] = Malloc_XXX_Log(pEngineParms, pEngineParms->CoreLogs[l][n], l, n, d, pDataParms->SampleCount);
			}
		}
	}
